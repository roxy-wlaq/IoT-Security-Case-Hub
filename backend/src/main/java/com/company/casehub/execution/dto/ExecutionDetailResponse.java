...
public record ExecutionDetailResponse(UUID projectTestCaseId, UUID projectId, UUID testCaseVersionId,
                                      ExecutionStatus executionStatus, SelectionMode selectionMode,
                                      boolean evidenceRequired, List<DecisionResponse> decisionPoints) {
    public record DecisionResponse(UUID id, int displayOrder, String name, String transitionType, List<UUID> targetMasterTestCaseIds) { }
}
