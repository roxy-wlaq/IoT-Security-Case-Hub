...
import { downloadProjectExport, getVersionAvailability, listCustomTestCases, reviewCapabilityUpdateRequest, submitTestCaseChangeRequest, upgradeProjectTestCaseVersion } from '@/features/project/api/projectApi';
...
describe('Batch 4 project APIs', () => {
  ...
  it('downloads the protected project export as a Blob', async () => { const blob = new Blob(['xlsx']); mocked.get.mockResolvedValue({ data: blob }); await expect(downloadProjectExport('project-1')).resolves.toBe(blob); expect(mocked.get).toHaveBeenCalledWith('/projects/project-1/export.xlsx', { responseType: 'blob' }); });
});
